...
class My_Router extends CI_Router 
{
    ...
    function _parse_routes()
    {
        ...
        // now try the ECart specific routing
...
//        print_r(array_values($this->uri->segments)); exit;
        $this->_set_request(array_values($this->uri->segments));
    }
	
	function _get_db_route($slug)
	{
	    $ret = DB()->select('slug,template')->where('slug',$slug)->get('pages')->row();
        if(!empty($ret)){
            if($ret->template=='projects' || $ret->template=='projects_sub1' || $ret->template=='courses'){
                return array('slug'=>$ret->slug,'route'=>'pages/'.$ret->template.'/'.$ret->slug);
            } else {
                return array('slug' => $ret->slug, 'route' => 'pages/page/' . $ret->slug);
            }
        } else {
            return array();
        }
//        print_r($aa);
//	    return DB()->select('slug,route')->where('slug',$slug)->get('categories')->row_array();

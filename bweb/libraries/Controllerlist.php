            ...
            foreach(glob(APPPATH . 'controllers/admin/*') as $controller)
            {
                ...
                if(is_dir($controller))
                {
                    ...
                    foreach(glob(APPPATH . 'controllers/' . $dirname . '/*') as $subdircontroller)
                    {
                        ...
                        if(count($aMethods) > 0):
                            ...
                            {
                                if($method != '__construct' && $method != 'get_instance' && $method != 'set_template'&& $method != 'view' && $method != $subdircontrollername)
                                {
                                    $aUserMethods[] = $method;
                                }
                            }
                ...
                } else if(pathinfo($controller, PATHINFO_EXTENSION) == "php")
                {
                    ...
                    if(is_array($aMethods))
                    {
                        foreach($aMethods as $method)
                        {
                            if($method != '__construct' && $method != 'get_instance' && $method != 'set_template' && $method != 'view'  && $method != $controllername)
                            {
                                $aUserMethods[] = $method;
                            }
                        }

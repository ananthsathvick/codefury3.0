...
class Secure extends Front_Controller
{
                ...
                if ($redirect == '') {
                    ...
                    if (config_item('enable_payment')) {
                        if ($user->payment_status == 1 && (date('Y-m-d', strtotime($user->expiry_date)) > date('Y-m-d'))) {
                            ...
                            $this->session->set_flashdata('message', 'Pay To continue');
                            $redirect = 'secure/payment';
                        }
                    } else {
                        $redirect = 'secure/my_account';
                    }
    ...
    function my_account()
    {
        ...
        if (config_item('enable_payment')) {
            if (($user->payment_status == 0)||($user->payment_status == 1 && (date('Y-m-d', strtotime($user->expiry_date)) <= date('Y-m-d')))) {
                $this->session->set_flashdata('message', 'Pay To continue');
                redirect(site_url('secure/payment'));
            }
        }
    ...
    function resume_template()
    {
        ...
        $data['resume'] = $this->Common_model->get_tbl_row('resume', array('user_id' => $this->user['id'], 'expired' => 0));
        $this->set_template('my');
        $this->view('my_account/change_temp', $data);
    }
    ...
    function choose_temp()
    {
        ...
        $cl_val = $_GET['val'];
        $id = $this->user['id'];
        $row = $this->Common_model->get_tbl_row('resume', array('user_id' => $id));
        $data['old_clr'] = $row->color_id;
        $data['old_tmp'] = $row->tmp_id;
        $data['old_val'] = $row->res_color;

        $save['user_id'] = $id;
        $save['color_id'] = $clr;
        $save['tmp_id'] = $tmp;
        $save['res_color'] = $cl_val;
        $id = $this->Common_model->save_tbl_res('resume', $save);

        //$this->session->set_flashdata('message', 'Resume Template Updated');

        $resume = $this->Common_model->get_tbl_row('resume', array('user_id' => $id, 'expired' => 0));
        //redirect('secure/'.(!empty($resume->user_id)) ? ('canditate-details?K=' . base64_encode($resume->id)) : '#');
        ...
        $data['resume'] = $this->Common_model->get_tbl_row('resume', array('user_id' => $id, 'expired' => 0));
        ...
        $file_view = $this->load->view('candidate_details', $data, true);
        $data['resume'] = $file_view;
    ...
    function undo_temp()
    {
        ...
        $save['res_color'] = $cl_val;
        $id = $this->Common_model->save_tbl_res('resume', $save);
        redirect('secure/resume_template');
    }
    ...
    function resform($id = false)
    {
        ...
        $this->load->helper('form');
        $this->form_validation->set_rules('name', 'Name', 'trim|required|max_length[150]|alpha_dash');
        $this->form_validation->set_rules('email', 'Email', 'trim|required|valid_email');
        $this->form_validation->set_rules('phone', 'Phone Number', 'trim|required|numeric|max_length[15]|alpha_dash');
        $this->form_validation->set_rules('mobile', 'Alternate Phone Number', 'trim|numeric|max_length[15]|alpha_dash');
        $this->form_validation->set_rules('industry', 'Industry', 'trim|numeric|alpha_dash');
        $this->form_validation->set_rules('tot_experience', 'Total Years Of Experience', 'trim|numeric|alpha_dash');
        $this->form_validation->set_rules('objectives', 'Objectives', 'trim|max_length[300]|alpha_dash');
        $this->form_validation->set_rules('languages', 'Languages', 'trim|required|max_length[150]|alpha_dash');
        $this->form_validation->set_rules('address', 'Address', 'trim|required|max_length[300]|alpha_dash');
        $this->form_validation->set_rules('city', 'City', 'trim|required|max_length[30]|alpha_dash');
        $this->form_validation->set_rules('state', 'State', 'trim|required|max_length[25]|alpha_dash');
        $this->form_validation->set_rules('gender', 'Gender', 'trim|required|alpha_dash');
        $this->form_validation->set_rules('passport_no', 'Passport Number', 'trim|alpha_dash');
        $this->form_validation->set_rules('pincode', 'Pincode', 'trim|numeric|max_length[6]|alpha_dash');
        $this->form_validation->set_rules('company_exp', 'Number of Companies you have worked', 'trim|numeric|max_length[1]|alpha_dash');
        $this->form_validation->set_rules('skills', 'Skills', 'trim|max_length[500]|alpha_dash');
        $this->form_validation->set_rules('expertise', 'Area of Expertise', 'trim|max_length[250]|alpha_dash');
        $this->form_validation->set_rules('awards[]', 'Awards', 'trim|alpha_dash');
        $this->form_validation->set_rules('achievements[]', 'Achievements', 'trim|alpha_dash');
        $this->form_validation->set_rules('hobbies', 'Hobbies', 'trim|alpha_dash');
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $save['tmp_id'] = $this->input->post('tmp_id');
            $save['status'] = 1;
            ...
            $from_name = $this->config->item('company_name');
            $this->Email_model->send_email($from, $to, $template, $mail_data, $from_name, $cc = false, $bcc = false, $reply_to = false, $attached = false);
            $this->session->set_flashdata('message', 'Resume saved and uploaded successfully');
            echo json_encode(array('success_msg_redirect' => site_url('secure/resume_template')));
        }
    }
    ...
    function resume_preview_admin($id = false)
    {
        ...
        $this->set_template('my');
        // $this->view('my_account/resume_preview', $data);
        redirect('canditate-details?K=' . base64_encode($data['resume']->id));
    }

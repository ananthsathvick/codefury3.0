...
class Employer extends Front_Controller
{
    ...
    public function register($ajax = false)
    {
        ...
        $this->form_validation->set_rules('phone', 'Contact Numberd', 'trim|required|max_length[120]|is_unique[company.phone]');
        $this->form_validation->set_rules('mobile', 'Alternate Number', 'trim|required|max_length[120]');
...
//            $to ='yugesh.bweb@gmail.com';
    ...
    public function my_account()
    {
        ...
        if($this->input->get('from')!='' && $this->input->get('to')!=''){
            ...
            $arr['where'] = 'company_id = '.$this->company['id'].' AND (view_on BETWEEN "'.$from_date.'" AND "'.$end_date.'" OR download_on BETWEEN "'.$from_date.'" AND "'.$end_date.'")';
    ...
    public function download_resume($id=false)
    {
        ...
        // $this->fpdf->cell(40, 10, 'Mycontent');
		// echo $this->fpdf->output($file_name, 'D');
		// exit;
		...
		// 	$this->pdf->render();
		// 	$this->pdf->stream("yugesh.pdf", array("Attachment"=>1));
		// 	exit;
...
// print_a(json_decode($data['resume']->work_exp, true),true);        
        ...
        if (!empty($data['resume'])) {
			...
			$this->pdf->render();
			$this->pdf->stream($file_name.'pdf', array("Attachment"=>1));

            //$filepath = 'uploads/resume';
            //pdf_create($file_view, $file_name, false, $filepath);
            //$data['resume']->id.'_resume.pdf';
        } else {
            return false;
		}
    ...
    public function canditate_details($id=false)
    {        
        $this->Company_model->is_logged_in(true);
